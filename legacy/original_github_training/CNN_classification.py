...
def calculate_metrics(labels, scores, cutoff=0.5, po_label=1):  # Calculate performance metrics at threshold 0.5
    ...
    my_metrics['MCC'] = (tp * tn - fp * fn) / np.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn)) if ( tp + fp) * ( tp + fn) * ( tn + fp) * ( tn + fn) != 0 else 'NA'
...
def run_ten_fold_cross_validation(sequences, labels, batch_size=64, epochs=200):
    """Run stratified 10-fold CV without using the independent test set."""
    ...
    for fold, (train_index, validation_index) in enumerate(
        splitter.split(encoded, label_array), start=1
    ):
        ...
        history = network.fit(
            ...
            validation_data=(encoded[validation_index], label_array[validation_index]),
            batch_size=batch_size,
            epochs=epochs,
            verbose=1,
            callbacks=[early_stopping],
        )
        scores = network.predict(
            encoded[validation_index], batch_size=batch_size
        ).reshape(-1)
        metrics = calculate_metrics(label_array[validation_index], scores)
        metrics.update({
            'fold': fold,
            'AUROC': roc_auc_score(label_array[validation_index], scores),
    ...
    metrics_frame.to_csv(
        os.path.join(result_path, 'classification_10fold_cv_metrics.csv'),
        index=False,
    )
    numeric_columns = [
        'AUROC', 'ACC', 'MCC', 'Recall', 'Precision', 'F1-score',
        'epochs_trained'
    ]
    summary = metrics_frame[numeric_columns].agg(['mean', 'std'])
    summary.to_csv(
        os.path.join(result_path, 'classification_10fold_cv_summary.csv')
    )
    print('Classification 10-fold cross-validation summary:')
    print(summary)
...
def evaluate(X, Y,X_vali, Y_vali, X_TEST, Y_TEST, batch_size=512, epochs=100,line_number_train=0,line_number_vali=0,line_number_test=0):
    ...
    print(Y)
    X_train, y_train = X, np.asarray(Y, dtype=np.int32)
...
def main():
    ...
    if os.path.isfile('test.csv') and os.path.isfile('train+vali.csv'):
        print('Using repository train+validation and independent test files')
        independent_test_sequence, independent_test_linenumber, independent_test_label = process('test.csv')
        train_vali_sequence, train_vali_linenumber, train_vali_label = process('train+vali.csv')
    elif os.path.isfile('test_+10.csv') and os.path.isfile('train+vali_+10.csv'):
        print('Using legacy train+validation and independent test files')
    ...
    X_train, x_vali, Y_train, y_vali = train_test_split(
        np.asarray(train_vali_sequence),
        np.asarray(train_vali_label, dtype=np.int32),
        test_size=0.2,
        random_state=RANDOM_SEED,
        stratify=np.asarray(train_vali_label, dtype=np.int32),
    )
    X_TEST = np.asarray(independent_test_sequence)
    Y_TEST = np.asarray(independent_test_label, dtype=np.int32)
    set_global_seed(RANDOM_SEED)
    ind_res, history, y_test = evaluate(
        X_train,
        Y_train,
        x_vali,
        y_vali,
